#include <bits/stdc++.h>
#include <list>

using namespace std;

vector<string> split_string(string);

//WARNING:
// DOES NOT COMPILE -- INCOMPLETE CODE
// did not finish building hash before I found a better way

// struct for hash table
struct hashNode
{
    string word;
    int instances = 0;    
}

vector<list<hashNode> hashTable;
int tableSize = 64;
hash<string> word_hash;

// Complete the checkMagazine function below.
void checkMagazine(vector<string> magazine, vector<string> note) 
{

    // REQUIREMENTS
    /*
        - must have some way to check if a word has been used 
            (if "me" only appears once in the mag, you cannot use "me" twice in the note)
            could be bool or deleting once used?
        - where it is in the magazine array
        - actual word
    */

    // map strings from magazine into hash table
    int code;
    bool found = true;

    tableSize = (magazine.size() / 4) * 3 ;

    for(int i = 0; i < magazine.size(); i++)
    {
        code = hashFunction(magazine[i]);
        mapHash(code, magazine[i]);
    }

    // look up strings from note via hash 
    for(int i = 0; i < note.size(); i++)
    {
        code = hashFunction(note[i]);
        found = useWord(code, note[i]);
        if(!found)
        {
            cout << "NO" << endl;
            return;
        }
    }
    
   cout << "YES" << endl;
   return;

   //46:51.16
}

int hashFunction(string key)
{
    int value = 0;
    for(int i = 0; i < key.size(); i++)
    {
        value += int(key[i]);
    }

    return value % tableSize;
}

void mapHash(int code, string key)
{
    
    hashTable[code].push_back(key);

}

bool useWord(int code, string word)
{
    //loop through all list elements found at table index, code
}


int main()
{
    string mn_temp;
    getline(cin, mn_temp);

    vector<string> mn = split_string(mn_temp);

    int m = stoi(mn[0]);

    int n = stoi(mn[1]);

    string magazine_temp_temp;
    getline(cin, magazine_temp_temp);

    vector<string> magazine_temp = split_string(magazine_temp_temp);

    vector<string> magazine(m);

    for (int i = 0; i < m; i++) {
        string magazine_item = magazine_temp[i];

        magazine[i] = magazine_item;
    }

    string note_temp_temp;
    getline(cin, note_temp_temp);

    vector<string> note_temp = split_string(note_temp_temp);

    vector<string> note(n);

    for (int i = 0; i < n; i++) {
        string note_item = note_temp[i];

        note[i] = note_item;
    }

    checkMagazine(magazine, note);

    return 0;
}

vector<string> split_string(string input_string) {
    string::iterator new_end = unique(input_string.begin(), input_string.end(), [] (const char &x, const char &y) {
        return x == y and x == ' ';
    });

    input_string.erase(new_end, input_string.end());

    while (input_string[input_string.length() - 1] == ' ') {
        input_string.pop_back();
    }

    vector<string> splits;
    char delimiter = ' ';

    size_t i = 0;
    size_t pos = input_string.find(delimiter);

    while (pos != string::npos) {
        splits.push_back(input_string.substr(i, pos - i));

        i = pos + 1;
        pos = input_string.find(delimiter, i);
    }

    splits.push_back(input_string.substr(i, min(pos, input_string.length()) - i + 1));

    return splits;
}
